...
import { hashForDependencyMap } from './update-package-json';
...
function hasNewDependenciesSinceLastBuild(projectRoot: string, packageChecksums: PackageChecksums): boolean {
...
export async function hasPackageJsonDependencyListChangedAsync(projectRoot: string): Promise<boolean> {
...
export async function installCocoaPodsAsync(projectRoot: string): Promise<boolean> {
    ...
    try {
        await packageManager.installAsync({ spinner: step as any });
